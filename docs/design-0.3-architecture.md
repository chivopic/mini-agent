...
class LimitsConfig(BaseModel):
    ...
    max_output_chars: int = Field(default=12_000, ge=1024)  # 与 0.2 AgentConfig 一致；不静默改成 16k
...
def load_session(session_id: str, sessions_dir: Path | None = None) -> SessionData | None:
    path = resolve_session_file(sessions_dir or get_default_sessions_dir(), session_id)
    ...
    try:
        if version == 1:
            ...
            messages = history_v1_to_messages(
                data.get("history") or [], created_at=meta.created_at
            )
...
`resolve_session_file` 先以 `[A-Za-z0-9][A-Za-z0-9_-]{0,127}` 校验 ID，再解析真实路径并确认目标仍在 session store；list/load/save/delete 共用。`save_session` 用 store 内随机临时文件原子替换，只写 v2（`schema_version: 2`, `messages`, `permission_memory`），**不**镜像 `history`。0.2 读 0.3 文件会失败；CHANGELOG 写明升级前 `cp -r ~/.mini-agent/sessions`。0.3 读 0.2 必须成功。下次 save 即升 v2。
...
class ToolContext:
    def __init__(self, workspace_root: Path, config: AgentConfig, cancel: threading.Event) -> None: ...
...
| --- | --- | --- | --- | --- |
| `get_repo_map` | readonly | read | 迁入 tools；**必须** `resolve_relative_path`，逐文件跳过越界 symlink（安全 bugfix，允许打破「行为冻结」） | |
...
5. shell 自动白名单必须是参数形状已验证的只读 argv，且无换行、替换、展开、管道、重定向或控制符 → `shell=False` ALLOW。解释器、测试运行器、通用 `find`、`rg --pre` 等可执行外部程序的形状不得自动放行。
6. 否则 ASK。
...
```python
class Agent:
    def __init__(self, config, llm_client, listener=None, session=None,
                 registry=None, permission=None, monotonic=time.monotonic): ...
